...
NOTEBOOK = ROOT / "notebooks" / "kaggle_codi_causal_subspace_distillation.ipynb"
BUILDER = ROOT / "scripts" / "build_kaggle_codi_causal_subspace_distillation_notebook.py"
...
def test_notebook_exposes_account_split_resume_and_aggregation():
    text = _text()
    assert "scripts/run_codi_causal_subspace_distillation.py" in text
    ...
    for artefact in ("summary.json", "selectors.json", "teacher_cache.pt", "predictions.jsonl"):
...
def test_notebook_runs_focused_tests_and_states_boundaries():
    ...
    assert "tests/test_causal_subspace_distillation_runner.py" in text
    for arm in ("none", "full", "variance", "relevance", "causal", "random"):
        assert arm in text
    assert "retain-only" in text and "norm-matched" in text
