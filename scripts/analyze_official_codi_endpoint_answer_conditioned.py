"""Build the final answer-conditioned endpoint decision report."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Analyze answer-conditioned official-CODI endpoint results."
    )
    ...
    report = build_answer_conditioned_final_report(basis, utility)
    _atomic_text(args.output, json.dumps(report, indent=2, sort_keys=True) + "\n")
    markdown = args.output.with_suffix(".md")
    _atomic_text(markdown, render_answer_conditioned_markdown(report))
