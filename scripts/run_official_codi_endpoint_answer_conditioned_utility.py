"""Held-out utility screen for answer-conditioned CODI endpoint directions."""
...
from src.mech.endpoint_answer_conditioned import (
    ...
    ANSWER_CONDITIONED_UTILITY_SCHEMA_VERSION,
...
def _completed_batch(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError(f"completed answer-conditioned batch mismatch: {path}")
    return payload
...
def _arm_loss(
    ...
    *,
    ...
    if name == "answer_conditioned":
        basis, target, mode = bases.answer_conditioned, teacher, "projected"
        ...
        basis = bases.answer_conditioned
        target = teacher.index_select(0, permutation)
        mode = "projected"
    elif name == "complement":
        basis, target, mode = bases.answer_conditioned, teacher, "complement"
    else:
        raise ValueError(f"unknown answer-conditioned arm {name!r}")
    return answer_conditioned_endpoint_loss(
        student,
        target,
        mode=mode,
...
def run(args: argparse.Namespace) -> dict:
    ...
    if int(basis_payload.get("schema_version", -1)) != ANSWER_CONDITIONED_SCHEMA_VERSION:
        raise RuntimeError("basis is not an answer-conditioned endpoint artifact")
    metadata = basis_payload["metadata"]
    if metadata.get("contract") != "answer_conditioned_colon_block_states_v1":
        raise RuntimeError("answer-conditioned endpoint contract changed")
    if metadata.get("native_parity_gate", {}).get("status") != "passed":
        raise RuntimeError("answer-conditioned basis lacks native parity")
    if basis_payload.get("selection", {}).get("status") != "candidate_selected":
        raise RuntimeError("no stable answer-conditioned candidate was selected")
    bases = answer_conditioned_bases_from_state(basis_payload["bases"])
    validate_answer_conditioned_bases(bases, require_candidate=True)
    ...
    if len(update_indices) != len(validation_indices) or not update_indices:
        raise RuntimeError("answer-conditioned utility partitions are invalid")
    ...
    if device.type != "cuda" and not args.allow_cpu:
        raise RuntimeError("answer-conditioned utility requires CUDA")
    ...
    bases = replace(
        bases,
        answer_conditioned=bases.answer_conditioned.to(device=device, dtype=dtype),
    ...
    request = {
        "schema_version": ANSWER_CONDITIONED_UTILITY_SCHEMA_VERSION,
        "analysis": "official_codi_endpoint_answer_conditioned_equal_norm_utility",
        ...
        "arms": list(ANSWER_CONDITIONED_ARMS),
    ...
    progress = tqdm(
        range(total_batches), unit="batch", desc="Answer-conditioned endpoint utility"
    )
    for batch_index in progress:
        ...
        with _amp_context(device, args.precision):
            update_output = scorer(
                update_batch, return_answer_endpoint_hidden=True
            )
        ...
        generator = torch.Generator(device="cpu").manual_seed(
            args.seed * 1_000_003 + batch_index * 10_007 + 202_608_04
        )
        ...
        arm_payloads = {}
        for arm_index, name in enumerate(ANSWER_CONDITIONED_ARMS):
            raw_gradients = (
                full_gradients
                if name == "full_blocks"
                    ...
                    losses[name],
                    parameters,
                    retain_graph=arm_index + 1 < len(ANSWER_CONDITIONED_ARMS),
                )
            )
            ...
            alignment = gradient_inner_product(
                validation_gradients, raw_gradients
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run answer-conditioned official-CODI endpoint utility."
    )
