"""Combine the primary and secondary endpoint TSV-C utility scopes."""
...
def _load_summary(path: Path) -> dict:
    ...
    if not resolved.is_file():
        raise FileNotFoundError(f"endpoint TSV-C scope summary is missing: {resolved}")
    payload = json.loads(resolved.read_text(encoding="utf-8"))
    if payload.get("gate", {}).get("supported") not in {True, False}:
        raise RuntimeError(f"endpoint scope summary is incomplete: {resolved}")
    return payload


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Combine official-CODI endpoint TSV-C utility reports."
    )
    parser.add_argument("--all-layers", required=True, type=Path)
    parser.add_argument("--layer11", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    args = parser.parse_args()

    report = combine_endpoint_scope_reports(
        _load_summary(args.all_layers),
    ...
    _atomic_text(markdown, render_endpoint_tsvc_markdown(report))
