"""CPU-only matched selector-specificity analysis."""
...
from src.mech.kv_selector_specificity import (
    ...
    render_selector_specificity_markdown,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Compare matched R-KV, uniform, and random teacher-trace selectors "
            "using cross-validated reduced-rank prediction."
        )
    )
    ...
    summary = analyze_selector_specificity(
        selector_reports,
    ...
    _atomic_text(
        render_selector_specificity_markdown(summary, metadata),
