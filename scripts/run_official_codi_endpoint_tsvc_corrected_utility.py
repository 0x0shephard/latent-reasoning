"""Held-out equal-update-norm utility screen for corrected CODI colon targets."""
...
from src.mech.endpoint_tsvc import bases_from_state, match_gradient_norm
from src.mech.endpoint_tsvc_corrected import (
    CORRECTED_ENDPOINT_ARMS,
    CORRECTED_ENDPOINT_SCOPES,
...
CORRECTED_UTILITY_RUN_SCHEMA_VERSION = 2
    ...
    with torch.no_grad(), _amp_context(device, precision):
        output = functional_call(
            ...
            (batch,),
            {
                ...
                "return_answer_endpoint_hidden": False,
            },
            strict=False,
        )
    return [float(value) for value in output.per_example_loss.detach().float().cpu()]
...
def _completed_batch(
    ...
    *,
    ...
    if (
        int(payload.get("schema_version", -1)) != CORRECTED_UTILITY_RUN_SCHEMA_VERSION
        ...
        raise RuntimeError(f"completed corrected endpoint batch mismatch: {path}")
    return payload
...
def _arm_loss(
    ...
    *,
    ...
    if name == "full":
        return corrected_endpoint_tsvc_loss(
            student,
            teacher,
            scope=scope,
            mode="full",
    ...
    if name == "learned_top77":
        ...
        raise ValueError(f"unknown corrected endpoint arm {name!r}")
    return corrected_endpoint_tsvc_loss(
        student,
        target,
        scope=scope,
...
def run(args: argparse.Namespace) -> dict:
    ...
    if args.scope not in CORRECTED_ENDPOINT_SCOPES:
    ...
    if int(basis_payload.get("schema_version", -1)) != CORRECTED_ENDPOINT_TSVC_SCHEMA_VERSION:
        raise RuntimeError("basis is not a corrected source-faithful artifact")
    basis_metadata = basis_payload["metadata"]
    if basis_metadata.get("contract") != "source_faithful_student_and_teacher_answer_colon_v2":
        raise RuntimeError("basis endpoint contract is not source-faithful v2")
    if basis_metadata.get("native_parity_gate", {}).get("status") != "passed":
        raise RuntimeError("basis was created without a passing native parity gate")
    bases = bases_from_state(basis_payload["bases"])
    validate_corrected_bases(bases)
    if bases.rank != 77:
        raise RuntimeError("corrected endpoint basis rank must be 77")
    ...
    if len(update_indices) != len(validation_indices) or not update_indices:
        raise RuntimeError("corrected update and validation partitions are invalid")
    ...
    if set(update_indices).intersection(validation_indices):
        raise RuntimeError("corrected update and validation partitions overlap")

    device = select_device(args.device)
    if device.type != "cuda" and not args.allow_cpu:
        raise RuntimeError("corrected endpoint utility requires CUDA")
    ...
    if basis_metadata.get("checkpoint_sha256") != load_report.checkpoint_sha256:
        raise RuntimeError("corrected basis and utility checkpoint hashes differ")
    ...
    if basis_metadata.get("dataset_fingerprint") != fingerprint:
        raise RuntimeError("corrected basis and utility dataset fingerprints differ")
    ...
    request = {
        "schema_version": CORRECTED_UTILITY_RUN_SCHEMA_VERSION,
        "analysis": "official_codi_endpoint_tsvc_corrected_equal_norm_utility",
        "contract": basis_metadata["contract"],
        "scope": args.scope,
        ...
        "native_parity_gate": basis_metadata["native_parity_gate"],
        ...
        "arms": list(CORRECTED_ENDPOINT_ARMS),
        "metric": "source-faithful SmoothL1 with unbiased teacher-std normalization",
        "auxiliary_norm_matching": "each arm matched to native full endpoint gradient",
    ...
    if manifest_path.is_file():
        ...
        if previous.get("request_sha256") != request_sha256:
            raise RuntimeError("output directory contains a different corrected utility request")
    manifest = {
        **request,
        "state": "running",
    ...
    progress = tqdm(
        range(total_batches),
        unit="batch",
        desc=f"Official CODI corrected endpoint utility ({args.scope})",
    )
    for batch_index in progress:
        ...
        if student is None:
            raise RuntimeError("corrected student colon endpoint is missing")
        ...
        losses = {
            ...
            for name in CORRECTED_ENDPOINT_ARMS
        }
        ...
        arm_payloads = {}
        for arm_index, name in enumerate(CORRECTED_ENDPOINT_ARMS):
            raw_gradients = (
                full_gradients
                if name == "full"
                    ...
                    losses[name],
                    parameters,
                    retain_graph=arm_index + 1 < len(CORRECTED_ENDPOINT_ARMS),
                )
            )
    ...
    report["analysis"] = "official_codi_endpoint_tsvc_corrected_scope_utility"
    ...
    print(f"[complete] corrected utility gate={report['gate']['status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run corrected source-faithful CODI endpoint TSV-C utility."
    )
    ...
    parser.add_argument("--scope", choices=CORRECTED_ENDPOINT_SCOPES, required=True)
